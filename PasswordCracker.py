...
def crack_hash(hash,wordlist = None,hash_type = 'md5', min_length = 0,max_length = 0,characters = string.ascii_letters + string.digits,max_workers =4):
    ...
    if wordlist:
        with open(wordlist, 'r') as f:
            ...
            print(f"[*] Cracling hash {hash} using {hash_type} with a list of {total_lines} passwords.")
            
            with ThreadPoolExecuter(max_workers = max_workers) as executer:
        ...
        print(f'[*] cracking hash {hash} using {hash_type} with generated passwords of lengths from {min_length} to {max_length}. Total combinations')
...
if __name__ == '__main__':
    ...
    parser.add_argument('hash', help = 'The has to crack.' )
